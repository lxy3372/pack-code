...
class Application(Tkinter.Frame):
    ...
    def create_source_row(self):
        ...
        self.source_input = Tkinter.Entry(self, textvariable=self.source_path).grid(row=0, column=2)
        self.source_btn = Tkinter.Button(self, text="路径选择", command=self.build_source_path()).grid(row=0, column=3)

    def create_des_row(self):
        self.des_row = Tkinter.Label(text=" 目标地址:").grid(row=1, column=0)
        self.des_input = Tkinter.Entry(self, textvariable=self.des_path).grid(row=1, column=2)
        self.des_btn = Tkinter.Button(self, text="路径选择", command=self.build_des_path()).grid(row=1, column=3)
